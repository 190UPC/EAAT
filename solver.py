...
class Solver():
    def __init__(self, model, cfg):
        ...
        self.folder_name = str(cfg.ckpt_name)
            ...
            for inputs in self.train_loader:
                ...
                if cfg.verbose and self.step % cfg.print_interval == 0:
                    with open('logs/{}/'.format(self.folder_name) + 'logs.txt', 'a') as f:
                        ...
                        mean_psnr, mean_ssim = self.evaluate(cfg.valid_data_path, scale=cfg.scale, num_step=self.step)

                        t2 = times.time()

                        self.writer.add_scalar("PSNR_{}x:".format(scale), mean_psnr, self.step)
                        self.writer.add_scalar("SSIM_{}x:".format(scale), mean_ssim, self.step)

                        print('-- PSNR_x{}: {:.5f} -- SSIM: {:.7f} -- Total_Loss: {:.5f}\n'
                              .format(scale, mean_psnr, mean_ssim, (self.mean_l1) / cfg.print_interval))
                        ...
                        f.write('Step: {}'
                                '--> PSNR_x{}:{:.5f} --> SSIM:{:.5f} -->{:.3f}m\n'
                                .format(self.step, scale, mean_psnr, mean_ssim, ((t2 - t1) / 60)))
    ...
    def evaluate(self, test_data_dir, scale=2, num_step=0):
        ...
        for step, inputs in enumerate(test_loader):
            ...
            # print(sr.shape)
            height = sr.size(2)  # 510
            width = sr.size(3)  # 510
            # print(height, width)
            # print(hr.size())
            # hr = hr[:, :, :height, :width]
            psnr = calc_psnr(sr, hr, scale, 1, benchmark=True)  # rgb_range 是个range
            ssim = calculate_ssim_pt(sr, hr, scale, test_y_channel=True)
            ssim = ssim.item()
            mean_psnr += psnr / len(test_data)
            mean_ssim += ssim / len(test_data)
