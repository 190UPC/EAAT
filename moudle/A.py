...
class EfficientAttention(nn.Module):
    ...
    def forward(self, input_):
        ...
        keys = self.keys(input_).reshape(
            (n, self.key_channels, h * w))  # reshape 为 (batch_size, key_channels, height * width)
        queries = self.queries(input_).reshape(n, self.key_channels,
                                               h * w)  # reshape 为 (batch_size, key_channels, height * width)
        values = self.values(input_).reshape(
            (n, self.value_channels, h * w))  # reshape 为 (batch_size, value_channels, height * width)
