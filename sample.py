...
def sample(net, dataset, cfg):
    ...
    for step, (hr, lr, name) in enumerate(dataset):
        ...
        ti = t2 - t1
        print("Saved {} ({}x{} -> {}x{}, {:.3f}s -- SSIM {:.4f})"
              .format(sr_im_path, lr.shape[1], lr.shape[2], sr.shape[1], sr.shape[2], ti,ssim))
    print('Average PSNR on scale X{} is {} '.format(cfg.scale, avg_psnr))
    print('Average SSIM on scale X{} is {} '.format(cfg.scale, avg_ssim))
...
def main(cfg):
    ...
    # inputs = torch.zeros((1, 3, 1280//4, 720//4)).cuda()
    # summary(net, inputs, 4)
